...
$(document).ready(function(){
	...
	/*// Shift + Left Click on cipher name in enabled cipher table
	$("body").on("click", ".phraseGemCiphName", function (e) { // tC - history table cell
		if (shiftIsPressed) {
			var val = $(this).find(".cipherHoverLabel").html(); // get cipher name from element
			for (i = 0; i < cipherList.length; i++) {
				if (cipherList[i].cipherName == val) toggleCipher(i); // deactivate current cipher
			}
			return false; // don't show menu
		}
	});*/
